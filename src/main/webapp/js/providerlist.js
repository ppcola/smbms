...
function deleteProvider(obj){
	$.ajax({
		type:"GET",
		url:path+"/sys/provider/delete.do",
...
$(function(){
	$(".viewProvider").on("click",function(){
		...
		var obj = $(this);
		window.location.href=path+"/sys/provider/view.do?proid="+ obj.attr("proid");
	});
	
	$(".modifyProvider").on("click",function(){
		var obj = $(this);
		window.location.href=path+"/sys/provider/modify.do?proid="+ obj.attr("proid");
	});
